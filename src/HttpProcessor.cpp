 ...
 * 通过content-length等判断
 */
bool HttpProcessor::isMessageComplete(const string &message)
{
    auto blankLinePos=message.find("\r\n\r\n");
    if(blankLinePos==string::npos)//报文头部不完整
        return false;
    else
    {
        auto contentLengthPos=message.find("Content-length: ");
        if(contentLengthPos==string::npos)//找不到Content-Length字段,就当作报文没有body
            return true;
        else
        {
            auto strLength=message.size();
            string number;
            for(auto i=contentLengthPos+16;i<strLength;i++)
            {
                if(message[i]<'0' || message[i]>'9')
                    break;
                number+=message[i];
            }
            //FIXME 这里判断报文长度是否正确用了小于等于，如果连着多个报文例如管线机制就会出问题
            if((stoi(number)+blankLinePos+4)<=strLength)//body长度正确
                return true;
            else//body长度错误
                return false;
        }
